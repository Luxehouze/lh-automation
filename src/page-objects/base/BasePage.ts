...
export class BasePage {
  ...
  get newsLetterPopup() {
    return this.page.getByText(/Never Miss An Update/i);
  }
  get closeNewsletterBtn() {
    return this.page
      .locator('[data-testid="newsletter-modal-close-button"]')
      .nth(1);
  }
  get CSATPopup() {
    return this.page.getByText("We'd love your");
  }
  get closeCSATBtn() {
    return this.page.locator('[data-testid="csat-modal-no-thanks-button"]');
  }
  get sortDropdown() {
    return this.page.getByText("Sort by");
  }
  get sortHighToLow() {
    return this.page.getByText("Price: High to Low");
  }
  get sortLowToHigh() {
    return this.page.getByText("Price: Low to High");
  }
  get sortAtoZ() {
    return this.page.getByText("Alphabetical, A - Z");
  }
  get sortZtoA() {
    return this.page.getByText("Alphabetical, Z - A");
  }
  get sortLatest() {
    return this.page.getByText("Latest");
  }

  public async closeNewsletterPopupIfVisible(
    maxWaitMs = 12000,
    intervalMs = 1000
  ): Promise<void> {
    ...
    while (Date.now() < deadline) {
      ...
      console.log(
        `Blocking check: titleVisible=${titleVisible}, overlayVisible=${overlayVisible}`
      );
  ...
  public async closeCSATPopupIfVisible(): Promise<void> {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      console.log(`CSAT title check [${i}] count=${count}, visible=${visible}`);

      if (visible) {
        console.log("CSAT popup detected. Trying to close...");

        const closeVisible = await closeBtn.isVisible().catch(() => false);
        console.log(
          "Close button CSAT count =",
          await closeBtn.count().catch(() => 0)
        );
        console.log("Close button CSAT visible =", closeVisible);

        if (closeVisible) {
          await closeBtn.click({ force: true }).catch(async () => {
            console.log("Close CSAT button click failed, pressing Escape...");
            await this.page.keyboard.press("Escape");
          });
        } else {
          console.log("Close CSAT button not found, pressing Escape...");
          await this.page.keyboard.press("Escape");
        }

        await this.page.getByText("We'd love your").waitFor({ state: 'detached', timeout: 5000
          })
          .catch(() => {});
        await this.page.waitForTimeout(500);
        console.log("CSAT popup closed.");
        return;
      }
    ...
    console.log("CSAT popup did not appear within timeout, skipping");
  }
